...
function registerEvents() {
  ...
  registerTwoPhaseEvent('onCompositionEnd', [TOP_BLUR, TOP_COMPOSITION_END, TOP_KEY_DOWN, TOP_KEY_PRESS, TOP_KEY_UP, TOP_MOUSE_DOWN]);
  registerTwoPhaseEvent('onCompositionStart', [TOP_BLUR, TOP_COMPOSITION_START, TOP_KEY_DOWN, TOP_KEY_PRESS, TOP_KEY_UP, TOP_MOUSE_DOWN]);
  registerTwoPhaseEvent('onCompositionUpdate', [TOP_BLUR, TOP_COMPOSITION_UPDATE, TOP_KEY_DOWN, TOP_KEY_PRESS, TOP_KEY_UP, TOP_MOUSE_DOWN]);
...
function extractCompositionEvent(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget) {
  ...
  var event = new SyntheticCompositionEvent(eventType, null, nativeEvent, nativeEventTarget);
...
function extractBeforeInputEvent(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget) {
  ...
  var event = new SyntheticInputEvent('onBeforeInput', null, nativeEvent, nativeEventTarget);
  accumulateTwoPhaseListeners(targetInst, dispatchQueue, event);
  event.data = chars;
}
/**
...
function disableLogs() {
  {
    if (disabledDepth === 0) {
      ...
      prevError = console.error; // https://github.com/facebook/react/issues/19099
...
function registerEvents$1() {
  registerTwoPhaseEvent('onChange', [TOP_BLUR, TOP_CHANGE, TOP_CLICK, TOP_FOCUS, TOP_INPUT, TOP_KEY_DOWN, TOP_KEY_UP, TOP_SELECTION_CHANGE]);
}
...
function extractEvents$2(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  var leave = new eventInterface(leaveEventType, from, nativeEvent, nativeEventTarget);
  leave.type = eventTypePrefix + 'leave';
  leave.target = fromNode;
  leave.relatedTarget = toNode;
  var enter = new eventInterface(enterEventType, to, nativeEvent, nativeEventTarget);
...
var rootTargetDependencies = [TOP_BLUR, TOP_CONTEXT_MENU, TOP_DRAG_END, TOP_FOCUS, TOP_KEY_DOWN, TOP_KEY_UP, TOP_MOUSE_DOWN, TOP_MOUSE_UP];
...
function isListeningToEvents(events, mountAt) {
  var listenerMap = getEventListenerMap(mountAt);

  for (var i = 0; i < events.length; i++) {
    var event = events[i];
    var capture = capturePhaseEvents.has(event);
    var listenerMapKey = getListenerMapKey(event, capture);
...
function isListeningToEvent(registrationName, mountAt) {
  var listenerMap = getEventListenerMap(mountAt);
  var capture = capturePhaseEvents.has(registrationName);
  var listenerMapKey = getListenerMapKey(registrationName, capture);
...
function extractEvents$3(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  if ( // We only listen to TOP_SELECTION_CHANGE on the document, never the
  // root.
  !isListeningToEvent(TOP_SELECTION_CHANGE, doc) || // If we are handling TOP_SELECTION_CHANGE, then we don't need to
  ...
  topLevelType !== TOP_SELECTION_CHANGE && !isListeningToEvents(rootTargetDependencies, targetContainer)) {
    return;
  }
...
function extractEvents$4(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  var event = new EventConstructor(reactName, null, nativeEvent, nativeEventTarget);
...
function extractEvents$5(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  extractEvents$4(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget);
...
function executeDispatchesInOrder(event, capture, bubble) {
  var previousInstance; // Dispatch capture phase first.

  for (var i = capture.length - 1; i >= 0; i--) {
    var _capture$i = capture[i],
        instance = _capture$i.instance,
        currentTarget = _capture$i.currentTarget,
        listener = _capture$i.listener;
  ...
  previousInstance = undefined; // Dispatch bubble phase second.

  for (var _i = 0; _i < bubble.length; _i++) {
    var _bubble$_i = bubble[_i],
        _instance = _bubble$_i.instance,
        _currentTarget = _bubble$_i.currentTarget,
        _listener = _bubble$_i.listener;
...
function dispatchEventsInBatch(dispatchQueue) {
  for (var i = 0; i < dispatchQueue.length; i++) {
    ...
    var event = dispatchQueueItem.event,
        capture = dispatchQueueItem.capture,
        bubble = dispatchQueueItem.bubble;
    executeDispatchesInOrder(event, capture, bubble); // Modern event system doesn't use pooling.
...
function listenToTopLevelEvent(topLevelType, target, listenerMap, eventSystemFlags, passive, priority, capture) {
  ...
  capture = capture === undefined ? capturePhaseEvents.has(topLevelType) : capture;
  var listenerMapKey = getListenerMapKey(topLevelType, capture);
  var listenerEntry = listenerMap.get(listenerMapKey);
  var shouldUpgrade = shouldUpgradeListener(listenerEntry, passive); // If the listener entry is empty or we should upgrade, then
  ...
  if (listenerEntry === undefined || shouldUpgrade) {
    ...
    if (shouldUpgrade) {
      removeTrappedEventListener(target, topLevelType, capture, listenerEntry.listener);
    }

    var listener = addTrappedEventListener(target, topLevelType, eventSystemFlags, capture, false, passive, priority);
...
function addTrappedEventListener(targetContainer, topLevelType, eventSystemFlags, capture, isDeferredListenerForLegacyFBSupport, passive, priority) {
  var listener = createEventListenerWrapperWithPriority(targetContainer, topLevelType, eventSystemFlags, priority); // If passive option is not supported, then the event will be
...
function createDispatchQueueItem(event, capture, bubble) {
  return {
    event: event,
    capture: capture,
    bubble: bubble
...
function accumulateTwoPhaseListeners(targetFiber, dispatchQueue, event, accumulateEventHandleListeners) {
  var bubbled = event._reactName;
  var captured = bubbled !== null ? bubbled + 'Capture' : null;
  var capturePhase = [];
  var bubblePhase = []; // If we are not handling EventTarget only phase, then we're doing the
    ...
    if (tag === HostComponent && stateNode !== null) {
      ...
      if (captured !== null) {
        ...
        if (captureListener != null) {
          capturePhase.push(createDispatchQueueItemPhaseEntry(instance, captureListener, currentTarget));
        }
      }
      ...
      if (bubbled !== null) {
        ...
        if (bubbleListener != null) {
          bubblePhase.push(createDispatchQueueItemPhaseEntry(instance, bubbleListener, currentTarget));
        }
      }
    }
  ...
  if (capturePhase.length !== 0 || bubblePhase.length !== 0) {
    dispatchQueue.push(createDispatchQueueItem(event, capturePhase, bubblePhase));
  }
}
...
function accumulateEnterLeaveListenersForEvent(dispatchQueue, event, target, common, capture) {
...
function accumulateEnterLeaveListeners(dispatchQueue, leaveEvent, enterEvent, from, to) {
...
function eagerlyTrapReplayableEvents(container, document) {
  ...
  var listenerMapForContainer = getEventListenerMap(container); // Discrete
...
function setInitialProperties(domElement, tag, rawProps, rootContainerElement) {
  ...
  switch (tag) {
    ...
    case 'input':
      ...
      props = getHostProps(domElement, rawProps); // For controlled components we always need to ensure we're listening
    ...
    case 'select':
      ...
      props = getHostProps$2(domElement, rawProps); // For controlled components we always need to ensure we're listening
      // to onChange. Even if there is no listener.
    ...
    case 'textarea':
      ...
      props = getHostProps$3(domElement, rawProps); // For controlled components we always need to ensure we're listening
      // to onChange. Even if there is no listener.
...
function diffHydratedProperties(domElement, tag, rawProps, parentNamespace, rootContainerElement) {
  ...
  switch (tag) {
    case 'input':
      initWrapperState(domElement, rawProps); // For controlled components we always need to ensure we're listening
      // to onChange. Even if there is no listener.
    ...
    case 'select':
      initWrapperState$1(domElement, rawProps); // For controlled components we always need to ensure we're listening
      // to onChange. Even if there is no listener.
    ...
    case 'textarea':
      initWrapperState$2(domElement, rawProps); // For controlled components we always need to ensure we're listening
      // to onChange. Even if there is no listener.
...
var SyncLanePriority = 16;
var SyncBatchedLanePriority = 15;
var InputDiscreteHydrationLanePriority = 14;
var InputDiscreteLanePriority = 13;
var InputContinuousHydrationLanePriority = 12;
var InputContinuousLanePriority = 11;
var DefaultHydrationLanePriority = 10;
var DefaultLanePriority = 9;
var TransitionShortHydrationLanePriority = 8;
var TransitionShortLanePriority = 7;
var TransitionLongHydrationLanePriority = 6;
var TransitionLongLanePriority = 5;
...
/*                        */
1;
var SyncUpdateRangeEnd = 1;
var SyncBatchedLane =
/*                 */
2;
var SyncBatchedUpdateRangeEnd = 2;
...
/*                    */
28;
var InputDiscreteUpdateRangeStart = 3;
var InputDiscreteUpdateRangeEnd = 5;
...
/*                  */
224;
var InputContinuousUpdateRangeStart = 6;
var InputContinuousUpdateRangeEnd = 8;
...
/*                          */
16128;
var DefaultUpdateRangeStart = 9;
var DefaultUpdateRangeEnd = 14;
...
/*                  */
1032192;
var TransitionShortUpdateRangeStart = 15;
var TransitionShortUpdateRangeEnd = 20;
...
/*                   */
66060288;
var TransitionLongUpdateRangeStart = 21;
var TransitionLongUpdateRangeEnd = 26;
var SelectiveHydrationLane =
/*          */
100663296;
var SelectiveHydrationRangeEnd = 27; // Includes all non-Idle updates

var UpdateRangeEnd = 27;
var NonIdleLanes =
/*                                 */
134217727;
...
/*                             */
939524096;
var IdleUpdateRangeStart = 28;
var IdleUpdateRangeEnd = 30;
var OffscreenLane =
/*                   */
1073741824;
var NoTimestamp = -1; // "Registers" used to "return" multiple values
// Used by getHighestPriorityLanes and getNextLanes:

var return_highestLanePriority = DefaultLanePriority;
var return_updateRangeEnd = -1;
...
function getNextLanes(root, wipLanes) {
  ...
  var equalOrHigherPriorityLanes = NoLanes;
  ...
  nextLanes = pendingLanes & equalOrHigherPriorityLanes; // If we're already in the middle of a render, switching lanes will interrupt
...
function hasUpdatePriority(lanes) {
...
function findUpdateLane(lanePriority, wipLanes) {
  switch (lanePriority) {
    ...
    case InputDiscreteLanePriority:
      {
        var _lane = findLane(InputDiscreteUpdateRangeStart, UpdateRangeEnd, wipLanes);

        if (_lane === NoLane) {
          _lane = InputDiscreteHydrationLane;
        }
    ...
    case InputContinuousLanePriority:
      {
        var _lane2 = findLane(InputContinuousUpdateRangeStart, UpdateRangeEnd, wipLanes);

        if (_lane2 === NoLane) {
          _lane2 = InputContinuousHydrationLane;
        }
    ...
    case DefaultLanePriority:
      {
        var _lane3 = findLane(DefaultUpdateRangeStart, UpdateRangeEnd, wipLanes);

        if (_lane3 === NoLane) {
          _lane3 = DefaultHydrationLane;
        }
    ...
    case IdleLanePriority:
      var lane = findLane(IdleUpdateRangeStart, IdleUpdateRangeEnd, wipLanes);
...
function findTransitionLane(lanePriority, wipLanes, pendingLanes) {
  if (lanePriority === TransitionShortLanePriority) {
    var lane = findLane(TransitionShortUpdateRangeStart, TransitionShortUpdateRangeEnd, wipLanes | pendingLanes);

    if (lane === NoLane) {
      lane = findLane(TransitionShortUpdateRangeStart, TransitionShortUpdateRangeEnd, wipLanes);
  ...
  if (lanePriority === TransitionLongLanePriority) {
    var _lane4 = findLane(TransitionLongUpdateRangeStart, TransitionLongUpdateRangeEnd, wipLanes | pendingLanes);

    if (_lane4 === NoLane) {
      _lane4 = findLane(TransitionLongUpdateRangeStart, TransitionLongUpdateRangeEnd, wipLanes);

      if (_lane4 === NoLane) {
        _lane4 = TransitionLongHydrationLane;
      }
    }
...
function findLane(start, end, skipLanes) {
  ...
  var bitsInRange = (1 << end - start) - 1 << start;
  var possibleBits = bitsInRange & ~skipLanes;
  var leastSignificantBit = possibleBits & -possibleBits;
...
function markRootFinished(root, remainingLanes) {
  ...
  while (lanes > 0) {
    var index = pickArbitraryLaneIndex(lanes);
    var lane = 1 << index; // Clear the expiration time
...
var workInProgressRootLatestProcessedEventTime = NoTimestamp;
...
function requestUpdateLane(fiber, suspenseConfig) {
  ...
  if (suspenseConfig !== null) {
    ...
    lane = findTransitionLane(transitionLanePriority, currentEventWipLanes, currentEventPendingLanes);
  } else {
    ...
    (executionContext & DiscreteEventContext) !== NoContext && schedulerPriority === UserBlockingPriority$1) {
      ...
      var lanePriority = schedulerPriorityToLanePriority(schedulerPriority);
    ...
    case RootSuspended:
      {
        ...
        var hasNotProcessedNewUpdates = workInProgressRootLatestProcessedEventTime === NoTimestamp;
    ...
    case RootSuspendedWithDelay:
      {
        ...
        !shouldForceFlushFallbacksInDEV()) {
          ...
          if (workInProgressRootLatestSuspenseTimeout !== NoTimestamp) {
            ...
            var eventTimeMs = workInProgressRootLatestProcessedEventTime;
    ...
    case RootCompleted:
      {
        ...
        if ( // do not delay if we're inside an act() scope
        !shouldForceFlushFallbacksInDEV() && workInProgressRootLatestProcessedEventTime !== NoTimestamp && workInProgressRootCanSuspendUsingConfig !== null) {
          // If we have exceeded the minimum loading delay, which probably
          // means we have shown a spinner already, we might have to suspend
          // a bit longer to ensure that the spinner is shown for
          // enough time.
          var _msUntilTimeout2 = computeMsUntilSuspenseLoadingDelay(workInProgressRootLatestProcessedEventTime, workInProgressRootCanSuspendUsingConfig);
...
function renderDidSuspendDelayIfPossible() {
  ...
  if (workInProgressRoot !== null && (hasUpdatePriority(workInProgressRootSkippedLanes) || hasUpdatePriority(workInProgressRootUpdatedLanes))) {
...
function pingSuspendedRoot(root, wakeable, pingedLanes) {
  ...
  if (workInProgressRoot === root && isSubsetOfLanes(workInProgressRootRenderLanes, pingedLanes)) {
    ...
    if (workInProgressRootExitStatus === RootSuspendedWithDelay || workInProgressRootExitStatus === RootSuspended && workInProgressRootLatestProcessedEventTime === NoTimestamp && now() - globalMostRecentFallbackTime < FALLBACK_THROTTLE_MS) {
